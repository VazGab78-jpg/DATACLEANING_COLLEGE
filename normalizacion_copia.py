...
dataframe = pd.read_excel("dataset_original.xlsx")


# ------------------------------------------------------------
# FUNCIÓN PARA NORMALIZAR EL TEXTO
# ------------------------------------------------------------
# Esta función recibe una columna del DataFrame y
...
for columna in columnas:
    ...
    dataframe[columna] = normalizar(dataframe[columna]).str.title()

    # Separamos cada celda en una lista de palabras.
    # Ejemplo: "Juan Garcia Lopez" -> ["Juan", "Garcia", "Lopez"]
    dataframe[columna] = dataframe[columna].str.split()

    # Recorremos cada fila por cada columna.
    for fila in dataframe[columna]:

        # Recorremos cada palabra utilizando su posición
        # dentro de la lista.
        for i in range(len(fila)):

            # Comparamos la palabra actual con la lista de excepciones.
    ...
    # Ejemplo: ["Juan", "de", "Garcia"] -> "Juan de Garcia"
    dataframe[columna] = dataframe[columna].apply(" ".join)


# ------------------------------------------------------------
# CREACIÓN DE UNA COPIA CON LAS COLUMNAS NECESARIAS
# ------------------------------------------------------------
# Creamos una copia que contiene únicamente las columnas de nombres
# y apellidos que serán utilizadas para obtener una lista de palabras únicas.
dataframe_copia = dataframe[["nombre", "ape_paterno", "ape_materno"]].copy()
...
for columna in columnas:
    ...
    palabras_separadas = separar_palabras(dataframe_copia[columna])
...
dataframe.to_excel("dataset_normalizado.xlsx", index=False)
